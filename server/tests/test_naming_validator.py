...
class TestLoadSchema:
    def test_loads_default_config(self):
        ...
        assert schema.segments[4].name == "unit"
        assert schema.segments[5].name == "id"
        assert schema.segments[5].required is True

    def test_min_max_segments(self):
        schema = load_schema()
        assert schema.min_segments == 6
        assert schema.max_segments == 6
...
class TestValidNames:
    ...
    def test_five_segments(self):
        result = validate_tag_name("MUN.L2.PMP001.Pressure.Bar")
        assert result.valid is False
        assert any("too few" in e.message.lower() for e in result.errors)

    def test_six_segments(self):
        result = validate_tag_name("MUN.L2.PMP001.Pressure.Bar.1")
        assert result.valid is True
        assert result.errors == []

    def test_six_segments_higher_id(self):
        result = validate_tag_name("Munich.Line2.Pump001.Pressure.Pa.42")
        assert result.valid is True
        assert result.errors == []

    def test_minimal_valid(self):
        result = validate_tag_name("A1.B2.C3.D4.E5.1")
        assert result.valid is True
...
class TestSegmentCount:
    ...
    def test_too_many_segments(self):
        result = validate_tag_name("A.B.C.D.E.F.G")
        assert result.valid is False
...
class TestPatternViolations:
    ...
    def test_special_characters(self):
        result = validate_tag_name("MUN.L2.PMP-001.Pressure.Bar.1")
        assert result.valid is False
        assert result.errors[0].segment == "equipment"

    def test_underscore_rejected(self):
        result = validate_tag_name("MUN.L2.PMP_001.Pressure.Bar.1")
        assert result.valid is False
